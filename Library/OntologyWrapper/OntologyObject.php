 ...
 *======================================================================================*/
	 ...
	 * referenced by offsets in the current object.
	 *
	 ...
	 */
	public function collectProperties( $doSubOffsets = TRUE )
	{
		//
		// Init local storage.
		//
		...
		iterator_apply( $iterator,
						...
						array( $iterator, & $tags, $doSubOffsets ) );
 ...
 *======================================================================================*/
	 ...
	 * @uses getOffsetTypes()
	 */
	protected function traverseProperties( \Iterator $theIterator,
													&$theTags,
													$doSubOffsets = TRUE )
	{
		//
		...
		if( ! in_array( $key, static::InternalOffsets() ) )
		{
			...
			if( in_array( kTYPE_STRUCT, $type ) )
			{
				...
				if( in_array( kTYPE_LIST, $kind ) )
				{
					...
					foreach( $list as $idx => $struct )
					{
						...
						iterator_apply( $iterator,
										array( $this, 'traverseProperties' ),
										array( $iterator, & $theTags, $doSubOffsets ) );
			...
			{
				//
				// Handle primitive structures.
				//
				if( in_array( kTYPE_LANGUAGE_STRINGS, $type ) )
				{
					//
					// Handle structure lists.
					//
					if( in_array( kTYPE_LIST, $kind ) )
					{
						...
						foreach( $list as $item )
						{
							...
							{
								//
								// Iterate language string offsets.
								//
