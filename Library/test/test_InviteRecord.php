...
try
{
	...
	if( $result[ kAPI_RESPONSE_STATUS ][ kAPI_STATUS_CRYPTED ] )
	{
		...
		$encoded = $result[ kAPI_RESULTS_DICTIONARY ];
		$decoded = $encoder->privateDecode( $encoded, $ext_priv_key );
		$decoded = json_decode( $decoded, TRUE );
		$result[ kAPI_RESULTS_DICTIONARY ] = $decoded;
	}
	...
	//
	$admin_id = $result[ kAPI_RESULTS_DICTIONARY ][ kAPI_DICTIONARY_IDS ][ 0 ];
	echo( kSTYLE_TABLE_PRE );
	echo( kSTYLE_ROW_PRE );
	echo( kSTYLE_HEAD_PRE );
	echo( 'User:' );
	...
	echo( '$result[ '
		 ...
		 .$user_finger
		 .' ];' );
	$invitation = $result[ kAPI_RESPONSE_RESULTS ]
						 [ OntologyWrapper\User::kSEQ_NAME ]
						 [ $user_finger ];
	echo( kSTYLE_HEAD_POS );
	echo( kSTYLE_ROW_POS );
