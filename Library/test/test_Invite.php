...
$pubkey = '/Library/WebServer/Private/pgrdg/pub.pem';
...
try
{
	...
	(
		...
		(
			kTAG_ENTITY_EMAIL => 'pippo@pappa.org',
			kTAG_NAME => 'Pippo Pappa',
			kTAG_ROLES => array( ':roles:user-invite' ),
			kTAG_ENTITY_PGP_KEY => 'KEY',
			kTAG_ENTITY_PGP_FINGERPRINT => 'FINGERPRINT',
		)
	);
	$encoder = new OntologyWrapper\Encoder();
	$encoded = $encoder->publicEncode( json_encode( $param ), file_get_contents( $pubkey ) );
