...
{
	protected function instantiateParent( $theParameter )	{	RETURN NULL;	}
...
try
{
	...
	echo( kSTYLE_TABLE_PRE );
	echo( kSTYLE_ROW_PRE );
	$dsn = "db-driver://db-user:db-pass@host:80/name?db-opt1=val1&db-opt2=val2&db-opt3&db-opt4";
	echo( kSTYLE_HEAD_PRE );
	var_dump( $dsn );
	echo( kSTYLE_HEAD_POS );
	echo( kSTYLE_ROW_POS );
	...
	//
	// Test collection with DSN.
	//
	echo( '<h4>Test collection with DSN<br /><i>should add server host, port and database</i></h4>' );
	echo( kSTYLE_TABLE_PRE );
	echo( kSTYLE_ROW_PRE );
	$dsn_db = "collection-name";
	echo( kSTYLE_HEAD_PRE );
	var_dump( $dsn_db );
	echo( kSTYLE_HEAD_POS );
	echo( kSTYLE_ROW_POS );
	echo( kSTYLE_ROW_PRE );
	echo( kSTYLE_HEAD_PRE.'$db = $test->Collection( $dsn_db );'.kSTYLE_HEAD_POS );
	$db = $test->Collection( $dsn_db );
	echo( kSTYLE_ROW_POS );
	echo( kSTYLE_ROW_PRE );
	echo( kSTYLE_DATA_PRE );
